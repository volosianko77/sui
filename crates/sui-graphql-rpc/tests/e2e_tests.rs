...
mod tests {
    ...
    async fn test_zklogin_sig_verify() {
        ...
        let query = r#"{ verifyZkloginSignature(bytes: $bytes, signature: $signature, intentScope: $intent_scope, author: $author ) { errors}}"#;
        let variables = vec![
            GraphqlQueryVariable {
        ...
        // a valid signature with tx bytes returns errors as None in response.
        let binding = res.response_body().data.clone().into_json().unwrap();
        ...
        let binding = res.response_body().data.clone().into_json().unwrap();
        let res = binding.get("verifyZkloginSignature").unwrap();
        assert!(res.get("errors").unwrap().is_array());
    }
