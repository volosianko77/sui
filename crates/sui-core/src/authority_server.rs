...
impl ValidatorService {
    pub async fn new(
        ...
        Self {
            ...
            metrics,
            traffic_controller: Arc::new(
                TrafficController::spawn(firewall_config, policy_config).await,
            ),
        }
    }
...
impl ValidatorService {
    ...
    async fn submit_certificate_impl(
        ...
        let executed = self
            .handle_certificate(request, false)
            .instrument(span)
            .await?;
        Ok(tonic::Response::new(SubmitCertificateResponse { executed }))
    }
