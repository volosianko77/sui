...
use crate::authority::authority_store::{
    ExecutionLockReadGuard, ExecutionLockWriteGuard, LockDetails, ObjectLockStatus, SuiLockResult,
};
...
impl WritebackCache {
    ...
    fn get_transaction_lock(
        ...
        match self.locked_transactions.entry(*obj_ref) {
            DashMapEntry::Vacant(vacant) => {
                let lock = self.store.get_locked_transaction(obj_ref, epoch_store)?;
                if let Some(lock_details) = &lock {
                    vacant.insert(lock_details.clone());
                }
                Ok(lock)
            }
            DashMapEntry::Occupied(occupied) => Ok(Some(occupied.get().clone())),
        }
    }

    // Returns Err if there is a lock conflict. Otherwise returns true iff the lock needs to be
    // overwritten (i.e. it is not identical to the current lock)
    fn check_should_overwrite_lock(
        obj_ref: &ObjectRef,
        prev_lock: &LockDetails,
        new_lock: &LockDetails,
    ) -> SuiResult<bool> {
        if new_lock.epoch < prev_lock.epoch {
            return Err(SuiError::ObjectLockedAtFutureEpoch {
                obj_refs: vec![], // filled in by caller
                locked_epoch: prev_lock.epoch,
                new_epoch: new_lock.epoch,
                locked_by_tx: prev_lock.tx_digest,
            });
        }

        let overwrite = if new_lock.epoch == prev_lock.epoch {
            if prev_lock.tx_digest != new_lock.tx_digest {
                info!(prev_tx_digest = ?prev_lock.tx_digest,
                    cur_tx_digest = ?new_lock.tx_digest,
                ...
                // locks are identical, no need to overwrite
                false
            }
        } else {
            // new lock is from a later epoch, so we need to overwrite
            true
        };
        Ok(overwrite)
    }
    ...
    fn try_set_transaction_lock(
        ...
        let mut occupied = match entry {
            DashMapEntry::Vacant(vacant) => {
                if let Some(lock_details) =
                    self.store.get_locked_transaction(obj_ref, epoch_store)?
                {
                    vacant.insert_entry(lock_details.clone())
                } else {
                    vacant.insert_entry(new_lock.clone())
                }
            }
        ...
        if Self::check_should_overwrite_lock(obj_ref, prev_lock, &new_lock)? {
            occupied.insert(new_lock);
        }
    ...
    fn clear_cached_locks(
        &self,
        locks: &[(ObjectRef, LockDetails)],
        _execution_lock: &ExecutionLockReadGuard<'_>,
    ) {
    ...
    fn acquire_transaction_locks(
        ...
        for (obj_ref, live_object) in owned_input_objects.iter().zip(live_objects.into_iter()) {
            ...
            match self.try_set_transaction_lock(obj_ref, new_lock.clone(), epoch_store) {
                Ok(()) => {
                    locks_to_write.push((*obj_ref, new_lock));
                }
                Err(mut e) => {
                    ...
                    self.clear_cached_locks(&locks_to_write, execution_lock);

                    if let SuiError::ObjectLockedAtFutureEpoch { obj_refs, .. } = &mut e {
            ...
            CacheResult::Hit((_, obj)) => {
                ...
                if obj_ref != actual_objref {
                    ...
                    Ok(match self.get_transaction_lock(&obj_ref, epoch_store)? {
                        Some(lock_info) => {
                            match Ord::cmp(&lock_info.epoch, &cur_epoch) {
...
impl ExecutionCacheWrite for WritebackCache {
    ...
    fn acquire_transaction_locks<'a>(
        ...
        std::future::ready(WritebackCache::acquire_transaction_locks(
            self,
            epoch_store,
            execution_lock,
            owned_input_objects,
